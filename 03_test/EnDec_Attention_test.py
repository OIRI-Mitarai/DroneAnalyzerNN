...
# 学習済みモデルの読み込み
model = load_model(r'./model/EnDec_Attention.keras', safe_mode=False)
...
for csv_file in csv_files:
    ...
    output_filename = f'results/EnDec-Attention_prediction_with_all_confidence_{csv_file.split(".")[0]}.csv'
    result_df.to_csv(output_filename, index=False)

    # 結果のグラフ化と保存
    plt.figure(figsize=(12, 6))
    plt.plot(y_pred, marker='o')
    plt.title(f'Prediction Results for {csv_file} (Encoder-Decoder+Attention)')
    plt.xlabel('Time Step')
    plt.ylabel('Predicted Class')
    plt.yticks(range(class_num))
    plt.grid(True)
    output_filename = f'results/test_prediction_EnDec-Attention_{csv_file.split(".")[0]}.png'
